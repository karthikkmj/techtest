...
public class DataHeaderServiceTests {
    ...
    @Test
    public void shouldUpdateBlockTypeAsExpected(){
        dataHeaderService.updateBlockType(expectedDataHeaderEntity.getBlocktype(), expectedDataHeaderEntity.getName());

        verify(dataHeaderRepositoryMock, times(1))
                .updateBlocktype(eq(expectedDataHeaderEntity.getBlocktype().name()), eq(expectedDataHeaderEntity.getName()));
    }

    @Test
    public void shouldGetHeaderByBlockNameAsExpected(){
        dataHeaderService.getHeaderByBlockName(expectedDataHeaderEntity.getName());

        verify(dataHeaderRepositoryMock, times(1)).findByName(eq(expectedDataHeaderEntity.getName()));
    }
