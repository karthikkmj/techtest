...
public class DataBodyServiceTests {
    ...
    public void shouldGetDataByBlockTypeAsExpected(){
        dataBodyService.getDataByBlockType(expectedDataBodyEntity.getDataHeaderEntity().getBlocktype());

        verify(dataStoreRepositoryMock, times(1))
                .findByDataHeaderEntity_Blocktype(eq(expectedDataBodyEntity.getDataHeaderEntity().getBlocktype()));
    }

    @Test
    public void shouldGetDataByBlockNameAsExpected(){
        dataBodyService.getDataByBlockName(expectedDataBodyEntity.getDataHeaderEntity().getName());

        verify(dataStoreRepositoryMock, times(1))
                .findByDataHeaderEntity_Name(eq(expectedDataBodyEntity.getDataHeaderEntity().getName()));
    }
