...
public class TestDataHelper {
    ...
    public static DataEnvelope createTestDataEnvelopeApiObject()  {
        DataBody dataBody = new DataBody(DUMMY_DATA, getMD5Hash(DUMMY_DATA));
    ...
    public static List<DataBodyEntity> createTestDataBodyEntities()  {
        List<DataBodyEntity> list = new ArrayList<>();
        list.add(createTestDataBodyEntity(createTestDataHeaderEntity(Instant.now())));
        list.add(createTestDataBodyEntity(createTestDataHeaderEntity(Instant.now())));
        return list;
    }

    public static DataEnvelope createTestDataEnvelopeApiObjectWithWrongChecksum()  {
        DataBody dataBody = new DataBody(DUMMY_DATA, getMD5Hash("WRONG_DATA"));
        DataHeader dataHeader = new DataHeader(TEST_NAME, BlockTypeEnum.BLOCKTYPEA);
